...
Each site gets its own package under `pkg/scrape/<site>/parser.go` implementing `scrape.Scraper` (`Scrape(year, month int) ([]model.Concert, error)`). `cmd/ninkyclaw/main.go` runs the selected scrapers, collects `[]model.Concert`, rates, sorts, prints. A scraper failure is logged and skipped, not fatal — partial results are expected output.
